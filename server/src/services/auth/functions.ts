...
export const getAdminToken = async () => {
  const res = await fetch(`${process.env.KEYCLOAK_SERVER_URL}/realms/master/protocol/openid-connect/token`, {
    method: "POST",
...
export const searchUser = async (adminToken: string, email: string) => {
  ...
  const searchResponse = await fetch(`${process.env.KEYCLOAK_SERVER_URL}/admin/realms/${process.env.KEYCLOAK_REALM}/users?email=${encodeURIComponent(email)}`, {
    headers: {
