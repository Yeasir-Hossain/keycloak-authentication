...
export const register = async (req: Request, res: Response) => {
  ...
  // Register the user
  const registerResponse = await fetch(`${process.env.KEYCLOAK_SERVER_URL}/admin/realms/${process.env.KEYCLOAK_REALM}/users`, {
    method: "POST",
    headers: {
  ...
  if (!registerResponse.ok) {
    ...
    throw new HttpError(error.error || error.errorMessage, 400, true);
  }
